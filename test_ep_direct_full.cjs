async function getEmbedPlayDirectFull(tmdbId, season, episode, isMovie) {
  ...
  if (isMovie) {
     ...
     const match = html.match(new RegExp(`data-contentid=["'](\\d+)["'][^>]*data-season=["']${season}["'][^>]*data-episode=["']${episode}["']`, 'i'));
     if (match) {
        contentId = match[1];
     }
...
getEmbedPlayDirectFull("66732", 1, 1, false).then(url => console.log("Result:", url));
