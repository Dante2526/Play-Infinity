...
webpackEmptyAsyncContext.id = 8574;
...
var express = __webpack_require__(2883);
...
// EXTERNAL MODULE: ./node_modules/cheerio/dist/esm/index.js + 83 modules
var esm = __webpack_require__(6369);
// EXTERNAL MODULE: ./node_modules/dotenv/lib/main.js
var main = __webpack_require__(1398);
var main_default = /*#__PURE__*/__webpack_require__.n(main);
// EXTERNAL MODULE: ./node_modules/express-rate-limit/dist/index.mjs
var dist = __webpack_require__(5866);
;// CONCATENATED MODULE: ./src/data/serverBlacklist.ts
/**
 * ============================================================================
 ...
 */
function assertServerAllowed(urlOrKey) {
 ...
 */
function filterAllowedServers(servers) {
    if (!Array.isArray(servers))
        return [];
    return servers.filter(srv => {
        if (srv.key && isServerBlacklisted(srv.key))
            return false;
        if (srv.url && isServerBlacklisted(srv.url))
            return false;
        if (srv.name && isServerBlacklisted(srv.name))
            return false;
        return true;
    });
}

;// CONCATENATED MODULE: ./server/services/mostWatched.ts


// Fallback safe for cjs and esm
const _dirname = typeof __dirname !== 'undefined' ? __dirname : process.cwd();
const INITIAL_MOST_WATCHED = [
    {
        id: 299534,
        tmdbId: 299534,
...
const mostWatchedMemoryCache = loadMostWatchedFromDisk();
...
async function executeAtomicSaveMostWatched() {
    ...
    try {
        const dir = __webpack_require__.ab + "data";
        await external_fs_default().promises.mkdir(__webpack_require__.ab + "data", { recursive: true });
        const filePath = getMostWatchedFilePath();
        ...
        await external_fs_default().promises.writeFile(tempPath, payload, "utf-8");
        // 2. Substitui o arquivo de destino de forma atômica
        try {
            await external_fs_default().promises.rename(tempPath, filePath);
        }
        catch {
            // Fallback para plataformas em que rename sobre arquivo existente requer cópia explícita
            await external_fs_default().promises.copyFile(tempPath, filePath);
            await external_fs_default().promises.unlink(tempPath).catch(() => { });
        }
    }
    catch (err) {
...
var index_min = __webpack_require__(1632);
;// CONCATENATED MODULE: ./server/utils/caches.ts

// 1. Anime Direct Stream Cache - Limit to 500 items, TTL 1 hour
const animeDirectStreamCache = new index_min/* LRUCache */.q({
    max: 500,
    ttl: 1000 * 60 * 60, // 1 hour
});
// 2. Vixsrc Stream Cache - Limit to 200 items, TTL 30 minutes
const vixsrcStreamCache = new index_min/* LRUCache */.q({
    max: 200,
    ttl: 1000 * 60 * 30, // 30 mins
});
// 3. Live Chunk Cache (Buffer-heavy) - Limit to 100 items, TTL 15 seconds
const liveChunkCache = new index_min/* LRUCache */.q({
    max: 100,
    ttl: 1000 * 15, // 15 seconds
});
// 4. Search Cache (Already existed, we will redefine if needed)
const searchCache = new index_min/* LRUCache */.q({
    max: 100,
    ttl: 1000 * 60 * 10,
});

;// CONCATENATED MODULE: ./server/utils/helpers.ts
...
const trackPlayRateLimits = new Map();
function checkTrackPlayRateLimit(ip, titleKey) {
    ...
    if (record.titleCooldown.size > 100) {
        for (const [key, ts] of record.titleCooldown.entries()) {
            if (now - ts > 120000)
                record.titleCooldown.delete(key);
        }
    }
...
function isSuperflixDetected(content, url = "") {
    ...
    if (url && superflixDomainRegex.test(url))
        return true;
    if (superflixDomainRegex.test(content))
        return true;
...
function isPrivateOrLocalIp(hostname) {
    ...
    if (/^169\.254\./.test(host))
        return true; // AWS/GCP/Azure link-local metadata (169.254.169.254)
    if (!host.includes("."))
        return true; // Hosts locais sem domínio público
    if (/\.(local|internal|lan|corp|home)$/i.test(host))
        return true;
    return false;
}
...
if (external_fs_default().existsSync(".env.local")) {
    main_default().config({ path: ".env.local" });
}
main_default().config();
const customStreams = [];
...
app.use(express_default().json({ limit: '10kb' }));
app.use(express_default().urlencoded({ extended: true, limit: '10kb' }));
// ========================================================
// PROTEÇÃO CONTRA DDOS (Camada de Aplicação)
// ========================================================
// Limite Global: Protege a renderização estática e recursos
const globalLimiter = (0,dist/* default */.Ay)({
...
const apiLimiter = (0,dist/* default */.Ay)({
    windowMs: 1 * 60 * 1000, // 1 minuto
...
app.post("/api/track-play", (req, res) => {
    try {
        ...
        let existing = mostWatchedMemoryCache.find((it) => (tmdbId && it.tmdbId === Number(tmdbId)) || (id && it.id === id) || it.title.toUpperCase() === normTitle);
        if (existing) {
            ...
            if (safePlayerUrl && (!existing.playerUrl || existing.playerUrl.includes("watchplay.shop")))
                existing.playerUrl = safePlayerUrl;
            if (safeImageUrl && !existing.imageUrl)
                existing.imageUrl = safeImageUrl;
            if (safeBackdropUrl && !existing.backdropUrl)
                existing.backdropUrl = safeBackdropUrl;
...
app.get("/api/player-diagnostics", async (req, res) => {
    const testUrl = req.query.url || "https://v1.watchplay.shop/tvshow/66732/1/1";
...
async function resolveAnimesOnline(title, episode = 1) {
    ...
    try {
        ...
        const targetEp = uniqueEps.find(u => u.includes(`-episodio-${epNum}/`) ||
            u.includes(`-ep-${epNum}/`) ||
            u.endsWith(`-${epNum}/`) ||
            u.endsWith(`/${epNum}/`)) || uniqueEps[0];
...
async function resolveDirectAnimeStream(tmdbId, season, episode, isMovie = false, animeTitle = "") {
    ...
    try {
        ...
        if (isSuperflixDetected(html, pageUrl))
            return null;
        let contentId = null;
        if (isMovie) {
    ...
    try {
        ...
        if (!validation.valid) {
            ...
            try {
                ...
                if (parsedHost === "hclod.qzz.io" || parsedHost.endsWith(".hclod.qzz.io") ||
        ...
        const referer = req.query.referer || (rawUrl.includes("vixsrc") || rawUrl.includes("vix-content") ? "https://vixsrc.to/" : "https://v1.watchplay.shop/");
...
app.get("/api/anime-stream", async (req, res) => {
    ...
    if (!animeTitle) {
        if (tmdbId === "46260")
            animeTitle = "Naruto";
        else if (tmdbId === "31910")
            animeTitle = "Naruto Shippuden";
        else if (tmdbId === "12971")
            animeTitle = "Dragon Ball";
        else if (tmdbId === "12609")
            animeTitle = "Dragon Ball Z";
        else if (tmdbId === "60625")
...
app.get("/api/watchplayer-stream", async (req, res) => {
    try {
        ...
        if (req.query.action_secure_sign) {
            const rawUrl = req.query.raw_url;
        ...
        if (html.includes("player-choice") ||
        ...
        html = html.replace(/artInstance = new Artplayer\(\{/g, `artInstance = new Artplayer({
            controls: [],
            hotkey: false,
            gesture: false,
        ...
        html = html.replace(/maxBufferLength:\s*10,\s*maxMaxBufferLength:\s*20/g, `enableWorker: true,
        ...
        html = html.replace(/<div class="players_select_container">/g, '<div class="players_select_container" style="display:none !important; opacity:0 !important; visibility:hidden !important; pointer-events:none !important;">');
        html = html.replace(/<div class="player_container">/g, '<div class="player_container visible" style="position:absolute !important; top:0 !important; left:0 !important; width:100% !important; height:100% !important; transition:none !important;">');
...
app.get("/api/myembed-stream", async (req, res) => {
    try {
        const rawId = req.query.id || req.query.url || "tt22084616";
        ...
        const type = req.query.type || (rawId.includes("serie") ? "tv" : "movie");
        ...
        if (id.startsWith("tt")) {
            try {
                ...
                if (!tmdbApiKey) {
                    ...
                    const findRes = await fetch(`https://api.themoviedb.org/3/find/${id}?api_key=${tmdbApiKey}&external_source=imdb_id`);
                    if (findRes.ok) {
    ...
    (function() {
      ...
      art.on("video:ended", notifyEnded);
        ...
        const looksBlocked = (html, status) => {
            ...
            if (lower.includes("cf-error-details") ||
...
if (process.env.NODE_ENV !== "production") {
    ...
    (async () => {
        const viteName = "vite";
        const { createServer: createViteServer } = await __webpack_require__(8574)(viteName);
    ...
    const distPath = __webpack_require__.ab + "dist";
    if (external_fs_default().existsSync(__webpack_require__.ab + "dist")) {
        app.use(express_default()["static"](distPath));
        app.get("*", (_req, res) => {
            res.sendFile(external_path_default().join(distPath, "index.html"));
        });
    }
}
